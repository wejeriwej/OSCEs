...
//instruction.text("In your history, you forgot to ask about the following things:");
    ...
    if (jobx<1) {
      countdownEl.innerHTML = "> The patient's job";
    }//to tell the user if they forgot to ask about the patient's job
        
    if (radiatex<1) {
      document.write("> Whether the pain radiated to anywhere");
    }//does it radiate anywhere
    
    if (ratepainx<1) {
      whatforgottosay.text("> The severity of the pain");
    }//rate the pain
    
    if (makeitworsex<1) {
      whatforgottosay.text("> Whether or not anything makes the pain worse");
    }//make it worse
    
    if (makeitbetterx<1) {
      whatforgottosay.text("> Whether or not anything makes the pain better");
    }//make it better
    
    if (walkdistancex<1) {
      whatforgottosay.text("> How far they can walk");
    }//how far can they walk
    
    if (ideasx<1) {
      whatforgottosay.text("> The patient's ideas");
    }//ideas
    
    if (concernsx<1) {
      whatforgottosay.text("> The patient's concerns");
    }//concerns
    
    if (expectationsx<1) {
      whatforgottosay.text("> The patient's expectations");
    }//expectations
    
    if (familyhistoryx<1) {
      whatforgottosay.text("> Family history");
    }//FHx
    
    if (smokingx<1) {
      whatforgottosay.text("> Whether the patient smoked");
    }//smoking
    
    if (alcoholx<1) {
      whatforgottosay.text("> Whether the patient drank any alcohol");
    }//alcohol
